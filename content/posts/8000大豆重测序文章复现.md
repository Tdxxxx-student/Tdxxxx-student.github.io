...
###### 采取先分型再合并策略，提高效率
```bash
# 创建gvcf.chr.list,一列为不同样品的同一染色体的vcf文件的绝对路径
...
gatk SelectVariants  -R $ref -V $rawvcf --select-type INDEL -O yxq.raw.indel.vcf
gatk SelectVariants  -R $ref -V $rawvcf --select-type SNP -O yxq.raw.SNP.vcf
...
-O filter.indel.vcf 1>filterindel1.e5.log
gatk SelectVariants \
