...
有参考基因组的物种的全基因组测序叫做重测序，没有参考基因组的物种的全基因组测序则需要从头组装。随着测序价格的降低，越来越多物种的参考基因组都已经测序组装完成。[Plant Genomes](https://www.plabipd.de/plant_genomes_pa.ep)<sup>[4]</sup>网站实时显示全基因组测序已经完成的植物，其中2012年以后爆发式增长。在群体遗传学研究中更多的是有参考基因组的物种，尤其是模式物种，植物中常见的是拟南芥、水稻和玉米。
...
我们将这种选择之后多态性降低的情况叫做**选择扫荡（Selective Sweep）**。检测选择扫荡的软件有SweeD<sup>[7]</sup>。选择扫荡有可能是人工选择的结果，如2014年 _Nature Genetics_ 关于非洲栽培稻的文章就使用了SweeD来检测非洲栽培稻基因组上受人工选择的区域<sup>[8]</sup>。
...
3KRGP文章就计算了水稻不同亚群间4号染色体部分区域上的 **π** 值<sup>[12]</sup>，能够看出控制水稻籽粒落粒性的基因 _Sh4_<sup>[13]</sup> 位置多态性在所有的亚群中都降低了。说明这个基因在所有的亚群中都是受到选择的，这可能是人工选择的结果。
...
**Fst** 值可以和GWAS的结果一起进行分析，**Fst** 超过一定阈值的区域往往和GWAS筛选到的位点是一致的，如2018年棉花重测序的文章<sup>[15]</sup>。
...
常用的软件有 _structure_ 和 _ADMIXTURE_<sup>[19]</sup>。两款软件给出的结果都是 **K** 值。一般选择最低的点为最终的 **K** 值。
...
MSMC（Multiple Sequentially Markovian Coalescent）<sup>[27]</sup>，底层算法很复杂，类似于PSMC。主要功能是推断有效群体大小和群体分离历史。

### LAMP

LAMP（Local Ancestry in Admixed Populations，混杂群体的局部族源推断），用于对每组相邻的SNP进行检测分析<sup>[28]</sup>，在运算速度和推断准确度上都有了质的飞跃。

### Treemix

用于推断群体分离和混合<sup>[29]</sup>。这种图和进化树长得特别相似，可以将得到的结果和进化树进行比较。
