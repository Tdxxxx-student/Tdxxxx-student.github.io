+++
date = '2026-02-03T16:53:01+08:00'
draft = false
title = '基因家族分析'
categories = ["基因家族"]
tags= ["多重序列比对","遗传进化树","motif分析","hmm结构与分析","理化性质分析","启动子序列分析","共线性分析"]
+++
# 基因家族分析流程与文章撰写思路
...
需要准备的数据主要是参考基因组数据，包括fasta格式的序列文件、gff或gtf格式的基因组注释文件、蛋白质序列文件（通常是每个转录本的蛋白序列）、cDNA序列等文件。如果有转录组数据的话进行对应的转录组分析即可。除开上述这些文件外，还需要适用的文件还有.hmm格式的文件。
...
在得到基因ID后需要提取蛋白序列进行后续的分析。在SMART或者Pfam或NCBI CDD确认这些基因是真真正正含有该结构域，没有的基因要剔除！在SMART中没有检索到结构域的基因在gene.not.in.SMART.txt中；在Pfam中全都是WRKY结构域，对应文件为Pfam.results.txt。
